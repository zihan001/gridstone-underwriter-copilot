...
window.MEMO = (function () {
  ...
  "narrative": {
    "scope": "This appraisal estimates market value for 33xx Signal Hill Heights SW as of June 1, 2026, using sales comparison methodology. The subject property is located in Calgary's west district with an assessed value of $985,000. The analysis draws from Open Calgary data for the subject property and utilizes synthetic comparable sales generated by the deterministic engine for market value estimation.",
    "selection": "The engine retrieved 9 potential comparables and selected 4 for analysis after rejecting 5. Rejections included C-E (405 days stale), C-G (East district non-adjacent), C-F and C-H (outlier pricing at 3.1 and 7.6 MAD low respectively), and C-I (duplicate of C-A). The final comparable set required tier-2 relaxation to meet minimum count thresholds, triggering a DEEP_WIDENING flag with associated confidence penalty of -0.10.",
    "adjustment": "Selected comparables exhibit gross adjustment percentages ranging from 4.0% to 7.3%, with net adjustments from -4.9% to 7.2%. Three comparables (C-A, C-B, C-C) originate from the same west district, while C-D draws from an adjacent district with corresponding location risk mitigation through distance-weighting. Comparable C-D exceeds the 120-day recency threshold and utilizes city-wide fallback time adjustment series due to lack of encoded district benchmark data.",
    "reconciliation": "The weighted analysis produces a value range of $988,000 to $1,010,000 with a 2.2% spread. Individual adjusted values span from $983,187 (C-B) to $1,011,695 (C-D). Weights range from 0.4358 (C-A) to 0.1274 (C-D), with the latter receiving reduced weighting due to recency and district concerns. The point estimate of $999,000 reflects the weighted contribution of all four comparables.",
    "confidence": "Analysis confidence registers as MODERATE with a score of 0.7238. Positive contributors include comparable count (+0.08), adjusted-value spread (+0.125), recency (+0.0382), and distance (+0.0345). Negative factors include widening depth (-0.1) and minor adjustment burden (-0.0039). The confidence assessment reflects the impact of tier-2 widening requirements and reliance on adjacent district data for one comparable.",
    "limiting": "This analysis is subject to several limiting conditions requiring reviewer attention. The STALE_COMP flag indicates C-D's time adjustment approximation using city-wide fallback data. The ADJACENT_DISTRICT_COMP flag documents location risk for C-D. The UNSUPPORTED_TIME_ADJ flag confirms approximate time adjustment methodology for the adjacent district comparable. These flags represent standard AIC documentation requirements rather than fatal limitations to the analysis."
  },
  "agentTrace": {
    "intake": {
      "source": "llm",
      "reasoning": "## Subject Property Record\n\n**Identity & Assessment:**\n- Address: 33xx Signal Hill Heights SW\n- Roll Number: 091-44-218-03\n- District: West\n- Coordinates: 51.0207, -114.1573\n- Assessed Value: $985,000 (2026 assessment)\n- Land Use: R-C1\n- Year Built: 1998 (Open Calgary)\n\n**Physical Characteristics:**\n- GLA: 1,769 sq ft (listing)\n- Lot Size: 5,608 sq ft (listing)\n- Bedrooms Above Grade: 3 (listing)\n- Full Baths: 2 (listing)\n- Half Baths: 1 (listing)\n- Basement Finished: 600 sq ft (listing)\n- Basement Walkout: [No district-typical value available]\n- Garage Type: Attached (listing)\n- Garage Stalls: 2 (listing)\n\n**Intake Reasoning:** The property was successfully grounded through Open Calgary data, establishing the address, district, assessed value, and year built. Most physical characteristics were clearly stated in the listing text including square footage, lot size, bedroom/bathroom counts, finished basement area, and garage details. The only field not specified in the listing was basement walkout status, but no district-typical fallback value was available for this field in the west district.",
      "calls": [
        {
    ...
    "sensitivity": {
      "source": "llm",
      "note": "Sensitivity probe of the $999,000 central indication (range $988,000–$1,010,000). Three deterministic re-runs were compared against the delivered result: an alternate lender profile, a forced wider comp search, and leave-one-out on the most heavily weighted comp (C-A). Per-scenario engine output: [lender profile -> gse_on] point $999,000 -> $999,000 (unchanged); range $988,000–$1,010,000 -> $988,000–$1,010,000; confidence MODERATE 0.72 -> MODERATE 0.72; selected 4 -> 4; fired flags 4 -> 4. [forced wider search (min_comp_count=6)] point $999,000 -> $999,000 (unchanged); range $988,000–$1,010,000 -> $988,000–$1,010,000; confidence MODERATE 0.72 -> LOW 0.44; selected 4 -> 4; fired flags 4 -> 5. [dropping comp C-A] point $999,000 -> $999,000 (unchanged); range $988,000–$1,010,000 -> $988,000–$1,010,000; confidence MODERATE 0.72 -> MODERATE 0.72; selected 4 -> 4; fired flags 4 -> 4. The lender profile is informational (it changes flag loudness, not value); the range movement under wider search and leave-one-out bounds how sensitive the point is to the comp set. Every figure here is recomputed by the core, not estimated.",
      "calls": [
        {
          "name": "rerun_with_profile",
          "args": "profile=fnma_off",
          "result": "[lender profile -> fnma_off] point $999,000 -> $999,000 (unchanged); range $988,000–$1,010,000 -> $988,000–$1,010,000; confidence MODERATE 0.72 -> MODERATE 0.72; selected 4 -> 4; fired flags 4 -> 4."
        },
        {
          "name": "rerun_with_profile",
          "args": "profile=gse_on",
          "result": "[lender profile -> gse_on] point $999,000 -> $999,000 (unchanged); range $988,000–$1,010,000 -> $988,000–$1,010,000; confidence MODERATE 0.72 -> MODERATE 0.72; selected 4 -> 4; fired flags 4 -> 4."
        },
        {
          "name": "rerun_widening",
          "args": "extra_tier=True",
          "result": "[forced wider search (min_comp_count=6)] point $999,000 -> $999,000 (unchanged); range $988,000–$1,010,000 -> $988,000–$1,010,000; confidence MODERATE 0.72 -> LOW 0.44; selected 4 -> 4; fired flags 4 -> 5."
        },
        {
        ...
        },
        {
          "name": "recompute_dropping_comp",
          "args": "comp_id=101",
          "result": "no comparable with id '101' in the universe; nothing dropped."
        },
        {
          "name": "recompute_dropping_comp",
          "args": "comp_id=0001",
          "result": "no comparable with id '0001' in the universe; nothing dropped."
        }
      ]
    }
  }
